...
+ (AudioStreamer*)sharedAudioStreamer
{
	static AudioStreamer* s_sharedAudioStreamer = nil;
	if(s_sharedAudioStreamer == nil) 
	{
		s_sharedAudioStreamer = [[AudioStreamer alloc] init];
	}
	return s_sharedAudioStreamer;
}
...
- (void)prepareWithURL:(NSURL *)aURL
{
	[url release];
	url = [aURL retain];
	lastProgress = 0;
...
- (void)failWithErrorCode:(AudioStreamerErrorCode)anErrorCode
{
	@synchronized(self)
	{
		...
		if (self.errorHandler) {
			[self.errorHandler audioStreamerEncounteredError:self];
		}
		else {
